...
if ($parentProcess.ProcessName -eq "cmd")
{
    ...
    Start-ComSpec -Arguments "/k cd ""$tempDirectoryPath"" & echo 1If you execute the command 'Initialize-CorePowerLatest', it will install the latest devtools. Please be aware that the 'Initialize-CorePowerLatest' command might potentially conflict with existing installations, so use it with caution."
    exit
}
